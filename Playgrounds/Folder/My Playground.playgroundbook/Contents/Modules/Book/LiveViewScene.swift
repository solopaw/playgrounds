...
public class LiveViewScene: SKScene, ToolbarViewControllerDelegate, UIGestureRecognizerDelegate, GraphicAccessibilityElementDelegate, BackgroundAccessibilityElementDelegate {
    ...
    func commonInit() {
        ...
        // Limit texture cache to this size.
        TextureCache.shared.sizeLimitMB = 72
    }
    ...
    public override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if UIAccessibility.isVoiceOverRunning, let placedGraphicsCount = graphicsPlacedCount {
            graphicsPlacedCount = nil
            UIAccessibility.post(notification: .announcement, argument: String(format: NSLocalizedString("%d graphics placed in scene", comment: "AX description of graphics placed on screen"), placedGraphicsCount))
        }

        // For tools that aren’t tracking touchMove events, we immediately remove the spark line if stepping is enabled.
        let toolSupportsTouchMoveEvents = selectedTool?.options.contains(.touchMoveEvents)
        if (toolSupportsTouchMoveEvents == nil || toolSupportsTouchMoveEvents == false) && steppingEnabled == true {
            resetSparkLinePath()
        }
    }
    ...
    func setButtonBarVisibility() {
        guard 
            let view = overlayHost,
            let buttonBar = buttonBar, 
            let toolBar = toolbarController.view as? ToolbarView 
            else { return }
        if (view.bounds.size.width - toolBar.bounds.size.width - controlsMargin) >= (controlsMargin + buttonBar.bounds.size.width + controlsMargin) {
            // Plenty of room for the buttons => show them.
            buttonBar.isHidden = false
        } else {
            // Insufficient room for the buttons => hide them.
    ...
    private func _addBGElement(frame: CGRect, label: String, elementCount: Int, quadrant: Quadrant) {
        ...
        if connectedToUserProcess {
            element.accessibilityHint = NSLocalizedString("Double touch to enable direct interaction", comment: "AX label")
        }
        // General Identifier for UI Automation
        element.accessibilityIdentifier = "LiveViewScene.\(quadrant)"
        axElements.append(element)
    }
    ...
    fileprivate func accessibilityActivate(element: BackgroundAccessibilityElement) -> Bool {
        if (!accessibilityAllowsDirectInteraction && connectedToUserProcess) {
            // This value persists until the user starts a drag and lifts their touch for at least one second.
