...
public class DynamicComposerViewController : UIViewController, PlaygroundLiveViewSafeAreaContainer, UIGestureRecognizerDelegate, LiveViewSceneDelegate {
    ...
    public override func viewDidLoad() {
        ...
        defaultBackgroundView.contentMode = .scaleAspectFill
        ...
        singleTouchPan = UIPanGestureRecognizer.init(target: self, action: #selector(handleSingleTouchPan(panGesture:)))
        singleTouchPan?.minimumNumberOfTouches = 1
        singleTouchPan?.maximumNumberOfTouches = 1
        singleTouchPan?.delaysTouchesBegan = true
        singleTouchPan?.cancelsTouchesInView = false
        singleTouchPan?.delegate = self
        pinchGesture?.require(toFail: singleTouchPan!)
        panGesture?.require(toFail: singleTouchPan!)
        skView.addGestureRecognizer(singleTouchPan!)
        
        NotificationCenter.default.addObserver(forName: UIAccessibility.voiceOverStatusDidChangeNotification, object: nil, queue: OperationQueue.main) { (note : Notification) in
    ...
    private func updateGestureAvailabilityForVoiceOver() {
        ...
        if UIAccessibility.isVoiceOverRunning || forceGesturesDisabled {
            pinchGesture?.isEnabled = false
            panGesture?.isEnabled = false
            singleTouchPan?.isEnabled = false
        }
        else {
            pinchGesture?.isEnabled = true
            panGesture?.isEnabled = true
            singleTouchPan?.isEnabled = true
        }
    }
    ...
    private var singleTouchPan : UIPanGestureRecognizer? = nil
    ...
    @objc(handleSingleTouchPan:)
    private func handleSingleTouchPan(panGesture: UIPanGestureRecognizer) {
        switch panGesture.state {
        case .began:
            // This is a bit on the clever side... the LiveViewScene is watching for raw touch events. However, we’re using gestures here to pan/zoom. Gestures have mechanisms to delay touchesBegan but that only happens if the gestures move into a failed state. In the case of just our pan/zoom gestures, a single touch gesture would never fail and the touches never get delivered. But by having a single-touch gesture, which both pan/zoom gestures require to fail, we can force both into a failed state by having the single-touch gesture recognize. Then we simply set the gesture’s state to failed to trigger delivery of the touch that had the single-touch gesture recognize in the first place.
    ...
    public override func viewDidLayoutSubviews() {
        liveViewScene.setButtonBarVisibility()
    }
}
