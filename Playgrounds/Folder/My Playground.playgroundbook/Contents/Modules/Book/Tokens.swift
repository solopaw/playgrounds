...
struct TokenIterator: IteratorProtocol {
    ...
    private static var numberFormatter: NumberFormatter = {
        ...
        // Use en_US number format irrespective of current locale, since that’s what will be used in code.
    ...
    let tokenizers = [
        ...
        // Match numbers.
        try! Tokenizer(pattern: "\\d{0,}\\.?\\d{1,}") { match in
            return .number(numberFormatter.number(from: match)?.doubleValue ?? 0.0)
        },
